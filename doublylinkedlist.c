...
void swap(struct node* ref1, struct node* ref2) {
	...
	if( ref1->next == ref2 ){
		...
		tempnode->prev = ref1->prev;	//4.prev = 3
		tempnode->next = ref1->next;	//4.next = 5
		
		ref1->prev->next = ref2;		// 3.next -> 5
		ref2->next->prev = ref1;		// 8.prev -> 4

		ref1->next = ref2->next;		// 4.next -> 5.next = 8			
		ref1->prev = ref2;				// 4.prev -> 5.prev = 4		
		
		ref2->prev = tempnode->prev;	// 5.prev -> 4.prev = 3
		ref2->next = ref1;				// 5.next -> 4.next = 5		
		
		
		//ref1->next->prev = ref2;		// 5.prev -> 5
		//ref2->prev->next = ref1;		// 4.next -> 4
		...
		tempnode->prev = ref2->prev;	//4.prev = 3
		tempnode->next = ref2->next;	//4.next = 5
		
		ref2->prev->next = ref1;		// 3.next -> 5
		ref1->next->prev = ref2;		// 8.prev -> 4

		ref2->next = ref1->next;		// 4.next -> 5.next = 8			
		ref2->prev = ref1;				// 4.prev -> 5.prev = 4		
		
		ref1->prev = tempnode->prev;	// 5.prev -> 4.prev = 3
		ref1->next = ref2;				// 5.next -> 4.next = 5		
	...
	}else if (	ref1->prev == NULL && ref2->next == NULL	){
		...
		ref1->next->prev = ref2;		//14.prev -> 55
		ref2->prev->next = ref1;		// 14.next -> 2
		

		ref2->next = ref1->next;		// 4.next -> 5.next = 8			
		ref2->prev = NULL;				// 4.prev -> 5.prev = 4		
		
		ref1->prev = tempnode->prev;	// 5.prev -> 4.prev = 3
		ref1->next = NULL;				// 5.next -> 4.next = 5		
		...
		ref1->prev = tempnode->prev;	// 5.prev -> 4.prev = 3
		ref1->next = tempnode->next;	// 5.next -> 4.next = 5		
		...
		ref2->prev->next = ref1;		//14.prev -> 55
		ref1->prev->next = ref2;		// 14.next -> 2
		ref1->next->prev = ref2;

		ref1->next = ref2->next;		// 4.next -> 5.next = 8			
		ref1->prev = NULL;				// 4.prev -> 5.prev = 4		
		
		ref2->prev = tempnode->prev;	// 5.prev -> 4.prev = 3
		ref2->next = tempnode->next;	// 5.next -> 4.next = 5		
		...
		tempnode->prev = ref1->prev;
		tempnode->next = ref1->next;
		
		//printf("\ntesing1 \n");
		
		ref1->prev->next = ref2;		// 2.next -> 12
		ref1->next->prev = ref2;		// 4.prev -> 12

		
		ref1->prev = ref2->prev;		// 3.prev -> 12.prev = 11
		ref1->next = ref2->next;		// 3.next -> 12.next = 14	
		
		ref2->prev->next = ref1;		// 11.next -> 3
		ref2->next->prev = ref1;		// 14.prev -> 3
		
		
		ref2->prev = tempnode->prev;
		ref2->next = tempnode->next;
...
int distance(struct node* ref1, struct node* ref2) {
	...
	if ( ref1 != ref2 ){
		...
		struct node* check = ref1;
		int checking = 0;			// check if ref1 is appear before ref2
		
		
		while ( check->next != NULL && checking == 0 ){
